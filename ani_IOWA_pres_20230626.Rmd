...
```{r linear_4, echo=TRUE, eval=TRUE, cache=FALSE, messages=FALSE, warning=FALSE}
...
make_frames(pos[1:100,], animate = TRUE, out_dir = myDir, ani_name = "ex1/ani1.mp4",
...
```{r nl_1.5, echo=TRUE, eval=TRUE, cache=FALSE, messages=FALSE, warning=FALSE, tidy=TRUE}
...
# make frames and create animation
make_frames(pos2, animate = TRUE, out_dir = myDir, ani_name = "ex0.5/ani1.mp4",
            show_progress = FALSE, overwrite = TRUE, preview = FALSE, cex = pos2$size, col = pos2$color)
...
```{r adv_7, echo = TRUE, eval = TRUE, message=FALSE, warning=FALSE, eval=TRUE, tidy=TRUE}
...
# create output file location
myDir <- file.path(myDir, "frames")
dir.create(myDir)

# set background limits to same extents as used when St. Mary's was cropped above. See ?make_frames for more help on arguments in "make_frames"
make_frames(pos[1:50,], recs=recs, background_ylim = c(45.904, 47), background_xlim = c(-84.7, -83.57), out_dir=myDir, animate = TRUE, preview = FALSE, tail_dur = 0,  ani_name = "ani2.mp4", overwrite=TRUE, show_progress=FALSE)
...
4. Create directory of frames and stitch into *.mp4 (loop)
...
if (dir.exists(myDir)) {unlink(myDir, recursive = TRUE)}
dir.create(myDir)
dir.create(file.path(myDir, "frames"))

# remind ourselves what pos object looks like (output from interpolate_path)
head(pos, n=3)
```
---
...
.panel[.panel-name[Table]
```{r bathy2.5, echo=FALSE, eval=TRUE, tidy=TRUE}
head(pos)
```
]
]
...
# St. Marys River sea lamprey
#### Show bathymetry, non-linear interpolation
...
for(i in 1:50){
  ...
  recs_i <- recs[recs$deploy_date_time <= t_seq[i] &
                   recs$recover_date_time >= t_seq[i], ]
  
  # plot frame
  png(file.path(myDir, "frames", pos_i$f_name[1]))
  ...
  points(pos_i$longitude, pos_i$latitude, pch = pos_i$pt, col = "black",
         bg = pos_i$color, cex = 3)
...
```{r bathy5.5, cache=FALSE, eval=TRUE, warning=FALSE, tidy=TRUE, message=FALSE}        
# stitch frames together into a video
make_video(input_dir = file.path(myDir, "frames"), input_ext = ".png", output = file.path(myDir, "frames", "ani3.mp4"), overwrite=TRUE)
...
.center[
<video width="640" height="360" autoplay loop>
	<source src="ex3/frames/ani3.mp4" type="video/mp4">
</video>
...
```{r ggplot3, echo=TRUE, tidy=TRUE, warning=FALSE, message=FALSE, error=FALSE, eval=TRUE}
...
ani <- ggplot() +
  ...
  geom_point(data = pos[1:50,], aes(x = longitude, y = latitude, group = animal_id, color = color), inherit.aes = FALSE, show.legend = FALSE) +
  xlab("longitude") +
  ylab("latitude") +
